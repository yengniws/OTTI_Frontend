...
//   content: string;
//   potId: number | null;
//   images: File[]; // Add images to the handle
// }
...
//     title,
//     content,
//     imageIds, // 이미지 ID 배열을 외부에서 접근 가능하게 함
//   }));
...
//         setImageIds(response.data); // 업로드된 이미지 ID를 저장
//       } catch (error) {
...
//   title: string;
//   content: string;
//   imageIds: string[]; // 이미지 ID 배열을 문자열로 변경
// }
...
//   const [content, setContent] = useState('');
//   const [images, setImages] = useState<File[]>([]);
//   const [imageIds, setImageIds] = useState<string[]>([]); // 문자열 배열로 수정
//   const fileInputRef = React.useRef<HTMLInputElement | null>(null);

//   useImperativeHandle(ref, () => ({
//     title,
//     content,
//     imageIds, // 문자열 배열을 외부에서 접근 가능하게 함
//   }));
